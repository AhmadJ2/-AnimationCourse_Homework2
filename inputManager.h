...
static void glfw_key_callback(GLFWwindow* window, int key, int scancode, int action, int modifier)
{
	...
	if (key == GLFW_KEY_ESCAPE && action == GLFW_PRESS)
		...
		case 'w':
		case 'W':
			rndr->core().camera_translation+=(Eigen::Vector3f(0, 0, 0.03f));
			break;
		case 's':
		case 'S':
			rndr->core().camera_translation += (Eigen::Vector3f(0, 0, 0.03f));
		...
		case 'R':
			break;
		case GLFW_KEY_UP:
			scn->data().velocity = Vector3d(0, 0.01, 0);
			break;
		case GLFW_KEY_DOWN:
			scn->data().velocity = Vector3d(0, -0.01, 0);
			break;
		case GLFW_KEY_LEFT:
			scn->data().velocity = Vector3d(-0.01, 0, 0);
			break;
		case GLFW_KEY_RIGHT:
			scn->data().velocity = Vector3d(0.01,0,0);
			break;
		...
		/*OUR SIMPLIFICATION*/
		case ' ':
			if (scn->activate_moving_objects) {
				scn->activate_moving_objects = false;
				rndr->core().is_animating = false;
				scn->data_list[0].prev_tree = scn->data_list[0].tree;
				scn->data_list[1].prev_tree = scn->data_list[1].tree;
			}
			else {
				scn->activate_moving_objects = true;
